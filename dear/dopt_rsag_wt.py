...
# ==============================================================================
"""
Experimental: RS&AG + Tensor Fusion with waiting times
"""
...
CYCLE_TIME = 5 # default: 5ms
THRESHOLD = 128  # default: 128mb
NUM_NEARBY_LAYERS = 8
...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, model, 
        ...
        Args:
            ...
            cycle_time: wait-in-buffer time threshold for tensor fusion. 
        """
        ...
        if size() > 1:
            ...
            if self._cycle_time is not None:
                self._num_nearby_layers = -1 # first merge all layers to record wait times
    ...
    def _reset_wait_times(self):
        self._param_wait_times = {} # get wait time by param name
        self._param_in_times = {}
        for p in self._register_parameters:
            p_name = self._param_names.get(p)
            self._param_wait_times[p_name] = 0
            self._param_in_times[p_name] = 0
    ...
    def _update_groups_with_wait_times(self):
        """
        Update tensor fusion groups according to the tensor wait-in-buffer times.
        Warning: it should be called between FF and BP due to buffer re-allocations.
        """
        assert len(self._module_groups) == 1, "Please first merge all layers to record wait times."
        # convert tensor wait time to module wait time
        module_wait_times = self._get_module_wait_times(mode='max')
        #if rank() == 0:
        #    print("module wait times (ms): ", module_wait_times)
        
        # clear buffers
        self._reset_wait_times()
        for pad_buffer in self._pad_buffers:
            del pad_buffer
        for shard_buffer in self._shard_buffers:
            del shard_buffer

        # set flags used to generate module groups
        time_cap = self._cycle_time
        flags = []
        for group_wt in module_wait_times:
            for i, wt in enumerate(group_wt):
                if i == 0 and time_cap == self._cycle_time:
                    flags.append(1.)
                elif i == 0 and time_cap > self._cycle_time:
                    flags.append(0.)
                    time_cap = self._cycle_time
                elif wt > time_cap:
                    flags.append(1.)
        ...
        # bcast flag results from rank 0 to others to gurantee the consistency
        flags = torch.tensor(flags).cuda()
        comm.bcast(flags, 0)
        comm.synchronize()
        #if rank() == 0:
        #    print("Tensor fusion with flags: ", flags)
        self._generate_groups_with_flags(flags, flag_at_begin=True)
    
    def _get_module_wait_times(self, mode='max'):
        """
        Convert tensor wait times to module wait times. 
        """
        module_wait_times = []
        for module_group in self._module_groups:
            ...
            for module in module_group:
                ...
                for p in self._module_direct_parameters[module_name]: 
                    p_name = self._param_names.get(p)
                    param_group.append(self._param_wait_times[p_name])
                if mode == 'max':
                    current_group.append(np.max(param_group))
                elif mode == 'mean':
                    current_group.append(np.mean(param_group))
                else: # raw
                    current_group.append(param_group)
            module_wait_times.append(current_group)
        return module_wait_times

    def _generate_groups_with_flags(self, flags, flag_at_begin=True):
        """
        Generate groups with boundary flags.
        Args:
            flags: 0-1 vector, with 1 indicating the bounadary between two groups
            flag_at_begin = True, [0, 1, 0] -> [[m1],[m2, m3]]; else [0, 1, 0] -> [[m1, m2], [m3]]
        """
        module_groups = []
        current_group = []
        for i, module in enumerate(self._register_modules):
            if flag_at_begin:
                # flags at the beginning of one group
                if flags[i] > 0 and len(current_group) > 0:
                    module_groups.append(current_group)
                    current_group = []
                current_group.append(module)
            else:
                # flags at the ending of one group
                current_group.append(module)
                if flags[i] > 0:
                    module_groups.append(current_group)
                    current_group = []
    ...
    def _push_to_buffer(self, name, tensor):
        ...
        with torch.no_grad():
            ...
            #if rank() == 0:
            #    print(self._param_group_flags)
            self._param_in_times[name] = time.time()
            for flag in self._param_group_flags[group_idx]:
                if flag == 0: # not ready
                    return name, None, None
            time_out = time.time()
            self._update_wait_times(time_out, group_idx)
    ...
    def _update_wait_times(self, time_out, group_idx, alpha=0.9):
        """
        Update the wait times when the group is ready.
        """
        for module in self._module_groups[group_idx]:
            module_name = self._module_names[module]
            for p in self._module_direct_parameters[module_name]:
                p_name = self._param_names[p]
                wait_time_old = self._param_wait_times[p_name]
                wait_time_new = (time_out - self._param_in_times[p_name]) * 1000 #ms
                self._param_wait_times[p_name] = (1-alpha) * wait_time_old + alpha * wait_time_new
    ...
    def _forward_pre_hook(self, module, input):
        ...
        if torch.is_grad_enabled() and self._num_steps > 0:
            ...
            if self._cycle_time is not None:
                if module == self._register_modules[-1] and self._num_steps == self._warmup:
                    self._update_groups_with_wait_times()
    ...
    def step(self, closure=None):
        ...
        #if self._num_steps == self._warmup * 2 and rank() == 0:
        #    print("module wait times:", self._get_module_wait_times(mode='raw'))
