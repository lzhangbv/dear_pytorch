...
"""
Pipelining reduce-scatter and all-gather tensor-wisely (which is slow due to heavy startup costs)
"""
...
from tensorfusion import MergedCommCollective, CollectiveOp, MergedCommReduce, CommReduceScatter
...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, model, compression,
        ...
        # dict used for reduce-scatter grad
        self._pad_num_dict = {}
        self._pad_grad_dict = {}
        self._shard_grad_dict = {}
    ...
    def _register_hooks(self):
        ...
        if self.exclude_reduce:
            return
        self._register_bp_hooks()

    def _get_pad_tensor(self, tensor, size):
        pad_num = size - tensor.numel() % size
        pad_tensor = tensor.new_empty(tensor.numel()+pad_num)
        shard_tensor = tensor.new_empty((tensor.numel()+pad_num) // size)
        ...
        for param_group in self.param_groups:
            for p in param_group['params']:
                if p.requires_grad:
                    ...
                    # padding tensor for reduce-scatter
                    with torch.no_grad():
                        pad_num, pad_tensor, shard_tensor = self._get_pad_tensor(p, size())
                    self._pad_num_dict[name] = pad_num
                    self._pad_grad_dict[name] = pad_tensor
                    self._shard_grad_dict[name] = shard_tensor
        ...
        if torch.is_grad_enabled():
            if not self.local and hasattr(module, 'weight'):
                ...
                if handles is not None:
                    grads = []
                    for handle, pad_tensor, pad_num, _ in handles:
                        if handle is not None and handle >= 0:
                            comm.syncStream(handle)
                        grads.append(pad_tensor[0:pad_tensor.numel()-pad_num])
    ...
    def _reduce_grad_async(self, p, name, r):
        ...
        """
        # change reduce to reduce_scatter
        tensor = p.data.view(-1) # flatten grad
        pad_num = self._pad_num_dict[name]
        pad_grad = self._pad_grad_dict[name]
        shard_grad = self._shard_grad_dict[name]

        # data copy
        numel = tensor.numel()
        pad_grad[0:numel].copy_(tensor.view(numel))

        handle = reduce_scatter_comm.collective_async_(name, pad_grad, shard_grad)
    ...
    def bcast_one_module(self, module):
        ...
        for i, next_tensor in enumerate(next_tensors):
            ...
            if i == 0 and hasattr(module, 'weight'):
                ...
                param_name = self._parameter_names.get(module.weight)
            if i == 1 and hasattr(module, 'bias'):
                tensor_name = name+'-bias'
                param_name = self._parameter_names.get(module.bias)

            if next_tensor is not None:
                # change to all-gather
                pad_num = self._pad_num_dict[param_name]
                pad_grad = self._pad_grad_dict[param_name]
                shard_grad = self._shard_grad_dict[param_name]
                ...
                handle = all_gather_comm.collective_async_(tensor_name, pad_grad, shard_grad)
                self._bcast_handles[module].append((handle, pad_grad, pad_num, 1))
